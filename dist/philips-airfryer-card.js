...
`,l2=n2+T2;function Z2(M,C){if(!M)return"";let H=C?.locale?.language||C?.language||"hu",V=C?.locale?.time_format,L={hour:"2-digit",minute:"2-digit"};V==="12"&&(L.hour12=!0),V==="24"&&(L.hour12=!1);try{return new Intl.DateTimeFormat(H,L).format(M)}catch{return M.toTimeString().slice(0,5)}}function A(M){return String(M??"").replace(/[&<>"']/g,C=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[C])}var U={language:"auto",compact:!1,animate:!0,show_methods:!0,show_presets:!0,show_settings:!0,show_probe:!0,show_details:!0,show_controls:!0};function S2(M,C){if(M==null)return null;let H=Math.max(0,Math.round(M));if(H>=3600){let r=Math.floor(H/3600),t=Math.round(H%3600/60);return{value:`${r}:${String(t).padStart(2,"0")}`,unit:C("ui.hour_short")}}let V=Math.floor(H/60),L=H%60;return{value:`${V}:${String(L).padStart(2,"0")}`,unit:C("ui.minute_short")}}var G=class extends HTMLElement{static getConfigElement(){return document.createElement(c)}static getStubConfig(C){let H=T(C);return{type:`custom:${v}`,device:H[0]?.id}}constructor(){super(),this.attachShadow({mode:"open"}),this._sections={},this._signatures={},this._built=!1}setConfig(C){this._config={...U,...C||{}},this._signatures={},this._hass&&this._render()}set hass(C){this._hass=C,this._render()}get hass(){return this._hass}connectedCallback(){this._timer=window.setInterval(()=>this._render(),3e4)}disconnectedCallback(){this._timer&&window.clearInterval(this._timer),this._timer=void 0}getCardSize(){return this._config?.compact?5:8}getGridOptions(){return{columns:12,min_columns:6,rows:this._config?.compact?6:"auto"}}_build(){let C=document.createElement("style");C.textContent=l2;let H=document.createElement("ha-card"),V=document.createElement("div");V.className="wrap",V.addEventListener("click",L=>this._onClick(L));for(let L of["header","hero","alerts","gauges","methods","presets","settings","controls","details"]){let r=document.createElement("div");r.dataset.section=L,this._sections[L]=r,V.appendChild(r)}H.appendChild(V),this.shadowRoot.replaceChildren(C,H),this._wrap=V,this._built=!0}_render(){if(!this._hass||!this._config)return;this._built||this._build();let C=y(B(this._config,this._hass)),H=p1(this._hass,this._config);if(this._model=H,!H.ok){this._renderEmpty(H,C);return}this._emptyShown=!1,this._applyHostClasses(H),this._section("header",this._headerHtml(H,C)),this._heroSection(H,C),this._section("alerts",this._alertsHtml(H,C)),this._section("gauges",this._gaugesHtml(H,C)),this._section("methods",this._config.show_methods?this._methodsHtml(H,C):""),this._section("presets",this._config.show_presets?this._presetsHtml(H,C):""),this._section("settings",this._config.show_settings?this._settingsHtml(H,C):""),this._section("controls",this._config.show_controls?this._controlsHtml(H,C):""),this._section("details",this._config.show_details?this._detailsHtml(H,C):"")}_renderEmpty(C,H){if(this._emptyShown!==C.reason){this._emptyShown=C.reason,this._signatures={};for(let V of Object.values(this._sections))V.innerHTML="";this._sections.header.innerHTML=`
      ...
      <div class="header">
        ...
        </div>`,this._statusNode=this._sections.hero.querySelector(".status"));let V=this._statusHtml(C,H);this._signatures.status!==V&&(this._signatures.status=V,this._statusNode.innerHTML=V);let L=S2(C.remaining,H),r=Z.includes(C.status)&&L&&C.remaining>0;m1(this._sections.hero,C.status===e.STANDBY||C.status===e.POWERSAVE?"":r?L.value:"--:--",C.currentTemp!==null&&C.status!==e.STANDBY?`${Math.round(C.currentTemp)}\xB0`:"")}_statusHtml(C,H){let V=[],L=C.status===e.PRECOOK&&C.targetTemp?`<span class="phase-text">${Math.round(C.targetTemp)} ${A(C.tempUnit)}</span>`:C.resting?`<span class="phase-text">${A(H("alert.resting"))}</span>`:"";V.push(`
      <div class="status-line">
        ...
        </div>`);let r=S2(C.remaining,H);if(Z.includes(C.status)&&r&&C.remaining>0){let t=C.finishAt?`<span class="at">${p("clock")}${A(H("ui.ready_at"))} ${Z2(C.finishAt,this._hass)}</span>`:"";V.push(`
        <div class="countdown">
...
`;function s2(M){return class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}setConfig(H){this._config={...H},this._render()}set hass(H){this._hass=H,this._render()}_render(){if(!this._config||!this._hass)return;let H=M.translator(M.getLanguage(this._config,this._hass)),V=M.listDevices(this._hass),L=this._config.device||"",r=[`<option value="" ${L?"":"selected"}>${A(H("editor.device_auto"))}</option>`,...V.map(i=>`<option value="${A(i.id)}" ${i.id===L?"selected":""}>${A(i.name)}</option>`)].join(""),t=[`<option value="auto">${A(H("editor.language_auto"))}</option>`,...M.languages.map(i=>`<option value="${i}" ${this._config.language===i?"selected":""}>${i.toUpperCase()}</option>`)].join(""),a=M.toggles.map(i=>{let o=this._config[i]??M.defaults[i]??!1;return`<label class="toggle"><input type="checkbox" data-key="${i}" ${o?"checked":""}>
        ...
        </div>`,this.shadowRoot.querySelectorAll("[data-key]").forEach(i=>i.addEventListener("change",o=>this._onChange(o)))}_onChange(H){let V=H.target,L=V.dataset.key,r={...this._config,type:this._config.type||`custom:${M.cardName}`};V.type==="checkbox"?r[L]=V.checked:V.value===""?delete r[L]:r[L]=V.value,this._config=r,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:r},bubbles:!0,composed:!0}))}}}var F2=s2({cardName:v,toggles:["show_methods","show_presets","show_settings","show_probe","show_details","show_controls","compact","animate"],defaults:U,listDevices:T,getLanguage:B,translator:y,languages:N});function c2(){customElements.get(c)||customElements.define(c,F2)}c2();u2();
