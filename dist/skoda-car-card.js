...
<svg class="car" viewBox="0 0 260 528" role="img" aria-hidden="true">
  <defs>
    <clipPath id="car-body"><path d="${_}"/></clipPath>
  ...
  <path class="contact" d="${_}" filter="url(#car-contact)"/>
  ...
  <path class="outline" d="${_}"/>

  ${Object.keys(t1).map(i5).join("")}
...
`;function K2(L){return class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}setConfig(H){this._config={...H},this._render()}set hass(H){this._hass=H,this._render()}_render(){if(!this._config||!this._hass)return;let H=L.translator(L.getLanguage(this._config,this._hass)),V=L.listDevices(this._hass),r=this._config.device||"",M=[`<option value="" ${r?"":"selected"}>${a(H("editor.device_auto"))}</option>`,...V.map(e=>`<option value="${a(e.id)}" ${e.id===r?"selected":""}>${a(e.name)}</option>`)].join(""),i=[`<option value="auto">${a(H("editor.language_auto"))}</option>`,...L.languages.map(e=>`<option value="${e}" ${this._config.language===e?"selected":""}>${e.toUpperCase()}</option>`)].join(""),t=L.toggles.map(e=>{let A=this._config[e]??L.defaults[e]??!1;return`<label class="toggle"><input type="checkbox" data-key="${e}" ${A?"checked":""}>
        ...
        </div>`,this.shadowRoot.querySelectorAll("[data-key]").forEach(e=>e.addEventListener("change",A=>this._onChange(A)))}_onChange(H){let V=H.target,r=V.dataset.key,M={...this._config,type:this._config.type||`custom:${L.cardName}`};V.type==="checkbox"?M[r]=V.checked:V.value===""?delete M[r]:M[r]=V.value,this._config=M,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:M},bubbles:!0,composed:!0}))}}}var d5=K2({cardName:Z,toggles:["show_alerts","show_drive","show_trip","show_service","show_charging","show_climate","show_system","show_extra","compact","animate"],defaults:I,listDevices:h,getLanguage:k,translator:w,languages:R});function q2(){customElements.get(O)||customElements.define(O,d5)}q2();$2();
