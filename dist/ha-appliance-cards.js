...
`,X2=e1+B5;function h(r,C){if(!r)return"";let H=C?.locale?.language||C?.language||"hu",V=C?.locale?.time_format,L={hour:"2-digit",minute:"2-digit"};V==="12"&&(L.hour12=!0),V==="24"&&(L.hour12=!1);try{return new Intl.DateTimeFormat(H,L).format(r)}catch{return r.toTimeString().slice(0,5)}}function E(r,C){if(r==null)return null;let H=Math.max(0,Math.round(r));if(H<60)return{value:String(H),unit:C("ui.minute_short")};let V=Math.floor(H/60),L=H%60;return{value:`${V}:${String(L).padStart(2,"0")}`,unit:C("ui.hour_short")}}function $(r,C){let H=E(r,C);return H?`${H.value} ${H.unit}`:""}function u1(r){if(r==null)return null;let C=Math.max(0,Math.round(r)),H=Math.floor(C/60),V=C%60;return H?`${H}:${String(V).padStart(2,"0")}`:`0:${String(V).padStart(2,"0")}`}function z(r,C=1){return r==null||!Number.isFinite(r)?"\u2013":r.toFixed(C).replace(/\.0$/,"")}function t(r){return String(r??"").replace(/[&<>"']/g,C=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[C])}var S1={language:"auto",compact:!1,animate:!0,show_programs:!0,show_options:!0,show_delay:!0,show_scores:!0,show_consumption:!0,show_details:!0,show_controls:!0,show_appliance_settings:!0},P5=[o.OFF,o.IDLE,o.READY_TO_START],J2=[o.IDLE,o.READY_TO_START],c1=class extends HTMLElement{static getConfigElement(){return document.createElement(W)}static getStubConfig(C){let H=r1(C);return{type:`custom:${k}`,device:H[0]?.id}}constructor(){super(),this.attachShadow({mode:"open"}),this._sections={},this._signatures={},this._built=!1}setConfig(C){this._config={...S1,...C||{}},this._signatures={},this._hass&&this._render()}set hass(C){this._hass=C,this._render()}get hass(){return this._hass}connectedCallback(){this._timer=window.setInterval(()=>this._render(),3e4)}disconnectedCallback(){this._timer&&window.clearInterval(this._timer),this._timer=void 0}getCardSize(){return this._config?.compact?5:9}getGridOptions(){return{columns:12,min_columns:6,rows:this._config?.compact?6:"auto"}}_build(){let C=document.createElement("style");C.textContent=X2;let H=document.createElement("ha-card"),V=document.createElement("div");V.className="wrap",V.addEventListener("click",L=>this._onClick(L));for(let L of["header","hero","alerts","programs","options","delay","meters","controls","appliance","details"]){let M=document.createElement("div");M.dataset.section=L,this._sections[L]=M,V.appendChild(M)}H.appendChild(V),this.shadowRoot.replaceChildren(C,H),this._card=H,this._wrap=V,this._built=!0}_render(){if(!this._hass||!this._config)return;this._built||this._build();let C=C1(this._config,this._hass),H=H1(C);this._t=H;let V=W1(this._hass,this._config);if(this._model=V,!V.ok){this._renderEmpty(V,H);return}this._emptyShown=!1,this._applyHostClasses(V),this._section("header",this._headerHtml(V,H)),this._heroSection(V,H),this._section("alerts",this._alertsHtml(V,H)),this._section("programs",this._config.show_programs?this._programsHtml(V,H):""),this._section("options",this._config.show_options?this._optionsHtml(V,H):""),this._section("delay",this._config.show_delay?this._delayHtml(V,H):""),this._section("meters",this._metersHtml(V,H)),this._section("controls",this._config.show_controls?this._controlsHtml(V,H):""),this._section("appliance",this._config.show_appliance_settings?this._applianceSettingsHtml(V,H):""),this._section("details",this._config.show_details?this._detailsHtml(V,H):"")}_renderEmpty(C,H){if(this._emptyShown!==C.reason){this._emptyShown=C.reason,this._signatures={};for(let V of Object.values(this._sections))V.innerHTML="";this._sections.header.innerHTML=`
      ...
      <div class="section">
        ...
        ${d("stop")}<span>${t(H("ui.delay_cancel"))}</span></button>`);let e=V?`<span class="hint">${t($(C.delay,H))}</span>`:`<span class="hint">${t(H("ui.delay_off"))}</span>`;return`
      <div class="section">
        <div class="section-title">${t(H("ui.delay"))}${e}</div>
        <div class="chips">${M.join("")}</div>
      </div>`}_metersHtml(C,H){let V=[],{eco:L,energy:M,water:e}=C.scores;if(this._config.show_scores&&(L!==null||M!==null||e!==null)){let a=(i,p,n,m)=>{let c=Math.max(0,Math.min(7,m??0)),Z=Array.from({length:7},(s,S)=>`<i class="${S<c?"on":""}"></i>`).join("");return`
          <div class="meter ${p}">
            <div class="top">${d(n)}<span>${t(H(`ui.${i}`))}</span>
              <span class="val">${m===null?"\u2013":`${m}/7`}</span></div>
            <div class="track">${Z}</div>
          </div>`};V.push(`
        <div class="meters">
          ${a("eco_score","eco","leaf",L)}
          ${a("energy_score","energy","energy",M)}
          ${a("water_score","water","water",e)}
        </div>`)}if(this._config.show_consumption&&C.program&&x[C.program]){let a=x[C.program];V.push(`
        <div class="facts">
          <span class="fact">${d("water")}<b>${z(a.water)}</b> l</span>
          <span class="fact">${d("energy")}<b>${z(a.energy,3)}</b> kWh</span>
          <span class="fact">${d("timer")}<b>${t($(a.duration,H))}</b></span>
        </div>`)}return V.length?`
      <div class="section">
        ...
        ${V.join("")}
          ...
          ${d(a.icon)}<span>${t(H(`command.${e}`))}</span></button>`});if(!L.length)return"";let M=C.remoteEnabled?"":`<div class="note">${d("remote")}${t(H("ui.remote_disabled_hint"))}</div>`;return`<div class="section"><div class="controls">${L.join("")}</div>${M}</div>`}_applianceSettingsHtml(C,H){let{brightness:V,floorLight:L,endSound:M,keyTone:e}=C.settings;if(!V&&!L&&!M&&!e)return"";let a=C.online&&C.remoteEnabled,i=(m,c,Z,s)=>s?`<button class="chip" type="button" data-action="setting" data-value="${m}"
            aria-pressed="${s.on}" title="${t(H(`ui.${Z}_hint`,""))}"
            ${a?"":"disabled"}>
...
`,v5=e1+$5;var f1={language:"auto",compact:!1,animate:!0,show_methods:!0,show_presets:!0,show_settings:!0,show_probe:!0,show_details:!0,show_controls:!0};function x5(r,C){if(r==null)return null;let H=Math.max(0,Math.round(r));if(H>=3600){let M=Math.floor(H/3600),e=Math.round(H%3600/60);return{value:`${M}:${String(e).padStart(2,"0")}`,unit:C("ui.hour_short")}}let V=Math.floor(H/60),L=H%60;return{value:`${V}:${String(L).padStart(2,"0")}`,unit:C("ui.minute_short")}}var O1=class extends HTMLElement{static getConfigElement(){return document.createElement(U)}static getStubConfig(C){let H=p1(C);return{type:`custom:${y}`,device:H[0]?.id}}constructor(){super(),this.attachShadow({mode:"open"}),this._sections={},this._signatures={},this._built=!1}setConfig(C){this._config={...f1,...C||{}},this._signatures={},this._hass&&this._render()}set hass(C){this._hass=C,this._render()}get hass(){return this._hass}connectedCallback(){this._timer=window.setInterval(()=>this._render(),3e4)}disconnectedCallback(){this._timer&&window.clearInterval(this._timer),this._timer=void 0}getCardSize(){return this._config?.compact?5:8}getGridOptions(){return{columns:12,min_columns:6,rows:this._config?.compact?6:"auto"}}_build(){let C=document.createElement("style");C.textContent=v5;let H=document.createElement("ha-card"),V=document.createElement("div");V.className="wrap",V.addEventListener("click",L=>this._onClick(L));for(let L of["header","hero","alerts","gauges","methods","presets","settings","controls","details"]){let M=document.createElement("div");M.dataset.section=L,this._sections[L]=M,V.appendChild(M)}H.appendChild(V),this.shadowRoot.replaceChildren(C,H),this._wrap=V,this._built=!0}_render(){if(!this._hass||!this._config)return;this._built||this._build();let C=A1(o1(this._config,this._hass)),H=n5(this._hass,this._config);if(this._model=H,!H.ok){this._renderEmpty(H,C);return}this._emptyShown=!1,this._applyHostClasses(H),this._section("header",this._headerHtml(H,C)),this._heroSection(H,C),this._section("alerts",this._alertsHtml(H,C)),this._section("gauges",this._gaugesHtml(H,C)),this._section("methods",this._config.show_methods?this._methodsHtml(H,C):""),this._section("presets",this._config.show_presets?this._presetsHtml(H,C):""),this._section("settings",this._config.show_settings?this._settingsHtml(H,C):""),this._section("controls",this._config.show_controls?this._controlsHtml(H,C):""),this._section("details",this._config.show_details?this._detailsHtml(H,C):"")}_renderEmpty(C,H){if(this._emptyShown!==C.reason){this._emptyShown=C.reason,this._signatures={};for(let V of Object.values(this._sections))V.innerHTML="";this._sections.header.innerHTML=`
      ...
      <div class="header">
        ...
        <div class="hero">
          ...
          <div class="status"></div>
        </div>`,this._statusNode=this._sections.hero.querySelector(".status"));let V=this._statusHtml(C,H);this._signatures.status!==V&&(this._signatures.status=V,this._statusNode.innerHTML=V);let L=x5(C.remaining,H),M=F.includes(C.status)&&L&&C.remaining>0;l5(this._sections.hero,C.status===A.STANDBY||C.status===A.POWERSAVE?"":M?L.value:"--:--",C.currentTemp!==null&&C.status!==A.STANDBY?`${Math.round(C.currentTemp)}\xB0`:"")}_statusHtml(C,H){let V=[],L=C.status===A.PRECOOK&&C.targetTemp?`<span class="phase-text">${Math.round(C.targetTemp)} ${t(C.tempUnit)}</span>`:C.resting?`<span class="phase-text">${t(H("alert.resting"))}</span>`:"";V.push(`
      <div class="status-line">
        ...
        </div>`);let M=x5(C.remaining,H);if(F.includes(C.status)&&M&&C.remaining>0){let e=C.finishAt?`<span class="at">${d("clock")}${t(H("ui.ready_at"))} ${h(C.finishAt,this._hass)}</span>`:"";V.push(`
        <div class="countdown">
      ...
      <div class="section">
        ...
        <div class="chips">${L.join("")}</div>
      </div>`}_settingsHtml(C,H){let V=i1.includes(C.status)||C.status===A.PAUSE,L=[],M=(i,p,n,m,c)=>{if(!n||!C.entities[c])return"";let Z=n.value??n.min,s=V&&Z>n.min,S=V&&Z<n.max;return`
        <div class="stepper">
