...
`,C5=i1+R5;function S(r,C){if(!r)return"";let H=C?.locale?.language||C?.language||"hu",V=C?.locale?.time_format,L={hour:"2-digit",minute:"2-digit"};V==="12"&&(L.hour12=!0),V==="24"&&(L.hour12=!1);try{return new Intl.DateTimeFormat(H,L).format(r)}catch{return r.toTimeString().slice(0,5)}}function W(r,C){if(r==null)return null;let H=Math.max(0,Math.round(r));if(H<60)return{value:String(H),unit:C("ui.minute_short")};let V=Math.floor(H/60),L=H%60;return{value:`${V}:${String(L).padStart(2,"0")}`,unit:C("ui.hour_short")}}function F(r,C){let H=W(r,C);return H?`${H.value} ${H.unit}`:""}function S1(r){if(r==null)return null;let C=Math.max(0,Math.round(r)),H=Math.floor(C/60),V=C%60;return H?`${H}:${String(V).padStart(2,"0")}`:`0:${String(V).padStart(2,"0")}`}function K(r,C=1){return r==null||!Number.isFinite(r)?"\u2013":r.toFixed(C).replace(/\.0$/,"")}function t(r){return String(r??"").replace(/[&<>"']/g,C=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[C])}var O1={language:"auto",compact:!1,animate:!0,show_programs:!0,show_options:!0,show_delay:!0,show_scores:!0,show_consumption:!0,show_details:!0,show_controls:!0,show_appliance_settings:!0},E5=[o.IDLE,o.READY_TO_START],H5=[o.IDLE,o.READY_TO_START],h1=class extends HTMLElement{static getConfigElement(){return document.createElement(Q)}static getStubConfig(C){let H=t1(C);return{type:`custom:${k}`,device:H[0]?.id}}constructor(){super(),this.attachShadow({mode:"open"}),this._sections={},this._signatures={},this._built=!1}setConfig(C){this._config={...O1,...C||{}},this._signatures={},this._hass&&this._render()}set hass(C){this._hass=C,this._render()}get hass(){return this._hass}connectedCallback(){this._timer=window.setInterval(()=>this._render(),3e4)}disconnectedCallback(){this._timer&&window.clearInterval(this._timer),this._timer=void 0}getCardSize(){return this._config?.compact?5:9}getGridOptions(){return{columns:12,min_columns:6,rows:this._config?.compact?6:"auto"}}_build(){let C=document.createElement("style");C.textContent=C5;let H=document.createElement("ha-card"),V=document.createElement("div");V.className="wrap",V.addEventListener("click",L=>this._onClick(L)),V.addEventListener("input",L=>this._onInput(L)),V.addEventListener("keydown",L=>this._onKeyDown(L));for(let L of["header","hero","alerts","programs","options","delay","meters","controls","appliance","details"]){let M=document.createElement("div");M.dataset.section=L,this._sections[L]=M,V.appendChild(M)}H.appendChild(V),this.shadowRoot.replaceChildren(C,H),this._card=H,this._wrap=V,this._built=!0}_render(){if(!this._hass||!this._config)return;this._built||this._build();let C=V1(this._config,this._hass),H=L1(C);this._t=H;let V=z1(this._hass,this._config);if(this._model=V,!V.ok){this._renderEmpty(V,H);return}this._emptyShown=!1,this._applyHostClasses(V),this._section("header",this._headerHtml(V,H)),this._heroSection(V,H),this._section("alerts",this._alertsHtml(V,H)),this._section("programs",this._config.show_programs?this._programsHtml(V,H):""),this._section("options",this._config.show_options?this._optionsHtml(V,H):""),this._section("delay",this._config.show_delay?this._delayHtml(V,H):""),this._section("meters",this._metersHtml(V,H)),this._section("controls",this._config.show_controls?this._controlsHtml(V,H):""),this._section("appliance",this._config.show_appliance_settings?this._applianceSettingsHtml(V,H):""),this._section("details",this._config.show_details?this._detailsHtml(V,H):"")}_renderEmpty(C,H){if(this._emptyShown!==C.reason){this._emptyShown=C.reason,this._signatures={};for(let V of Object.values(this._sections))V.innerHTML="";this._sections.header.innerHTML=`
      ...
      <div class="status-line">
        ...
        ${L}
      </div>`),C.program&&C.state!==o.OFF&&V.push(`
        <div class="program-line">
          ${d(x[C.program]?.icon||"dishwasher")}
          <span>${t(H(`program.${C.program}`))}</span>
        </div>`);let M=this._countdownHtml(C,H);return M&&V.push(M),[o.RUNNING,o.PAUSED,o.END_OF_CYCLE].includes(C.state)&&(V.push(`<div class="bar"><i style="width:${Math.round(C.progress*100)}%"></i></div>`),V.push(this._timelineHtml(C,H))),V.join("")}_cycleMinutes(C){let H=x[C.program]?.duration??null,V=C.remaining!==null&&C.remaining>0?C.remaining:null;if(![o.IDLE,o.READY_TO_START].includes(C.state))return V??H;let L=this._durationTrack;return L?L.program!==C.program?this._durationTrack={program:C.program,reported:V,stale:!0}:L.stale&&V!==L.reported&&(L.stale=!1):this._durationTrack={program:C.program,reported:V,stale:!1},this._durationTrack.stale?H??V:V??H}_countdownHtml(C,H){if(C.state===o.END_OF_CYCLE)return`<div class="countdown"><span class="at">${d("check")}${t(H("ui.finished_hint"))}</span></div>`;if(C.state===o.DELAYED_START){let V=W(C.delay||C.remaining,H);if(!V)return"";let L=[C.startAt?`<span class="at">${d("timer")}${t(H("ui.starts_at"))} ${S(C.startAt,this._hass)}</span>`:"",C.finishAt?`<span class="at">${d("clock")}${t(H("ui.ready_at"))} ${S(C.finishAt,this._hass)}</span>`:""].join("");return`<div class="countdown"><span class="value">${V.value}</span><span class="unit">${V.unit}</span>${L}</div>`}if([o.RUNNING,o.PAUSED].includes(C.state)){let V=W(C.remaining,H);if(!V)return"";let L=C.finishAt?`<span class="at">${d("clock")}${t(H("ui.ready_at"))} ${S(C.finishAt,this._hass)}</span>`:"";return`<div class="countdown"><span class="value">${V.value}</span><span class="unit">${V.unit}</span>${L}</div>`}if([o.IDLE,o.READY_TO_START].includes(C.state)){let V=this._cycleMinutes(C),L=W(V,H);if(!L)return"";let M=C.delay?new Date(Date.now()+C.delay*6e4):null,e=new Date(Date.now()+(V+C.delay)*6e4),a=[M?`<span class="at">${d("timer")}${t(H("ui.starts_at"))} ${S(M,this._hass)}</span>`:"",`<span class="at">${d("clock")}${t(H("ui.ready_at"))} ${S(e,this._hass)}</span>`].join("");return`<div class="countdown"><span class="value">${L.value}</span><span class="unit">${L.unit}</span>${a}</div>`}if(C.state===o.OFF&&C.delay&&C.program&&x[C.program]){let V=x[C.program].duration,L=W(V,H),M=new Date(Date.now()+C.delay*6e4),e=new Date(Date.now()+(V+C.delay)*6e4);return`<div class="countdown"><span class="value">${L.value}</span><span class="unit">${L.unit}</span>
        <span class="at">${d("timer")}${t(H("ui.starts_at"))} ${S(M,this._hass)}</span>
        <span class="at">${d("clock")}${t(H("ui.ready_at"))} ${S(e,this._hass)}</span></div>`}return""}_timelineHtml(C,H){let V=x[C.program]?.steps||v1,L=C.step?V.indexOf(C.step):-1;return`<div class="timeline">${v1.map(e=>{let a=V.includes(e),i=V.indexOf(e),A="step";return a?L>=0&&i<L?A+=" done":L>=0&&i===L?A+=" active":C.state===o.END_OF_CYCLE&&(A+=" done"):A+=" skip",`<div class="${A}"><span class="dot"></span><span class="label">${t(H(`phase.${e}`))}</span></div>`}).join("")}</div>`}_alertsHtml(C,H){let V=[];C.doorOpen&&C.state!==o.OFF&&V.push({severity:"warning",text:H("ui.door_open"),iconName:"door"});for(let L of C.alerts){let{code:M,count:e}=L,i=(L.severity==="ERROR"||L.severity==="CRITICAL"?"error":"")||s1[M]||"warning",A=M?H(`alert.${M}`,`${H("alert_generic")}: ${M.replace(/^DISH_ALARM_/,"")}`):`${H("alert_generic")}: ${e}`;V.push({severity:i,text:A,iconName:i==="error"?"error":"alert"})}return C.online||V.push({severity:"error",text:H("ui.offline"),iconName:"wifi_off"}),V.length?`<div class="alerts">${V.map(L=>`<div class="alert ${L.severity}">${d(L.iconName)}<span>${t(L.text)}</span></div>`).join("")}</div>`:""}_programsHtml(C,H){if(!C.entities.program||!C.programOptions.length)return"";let V=E5.includes(C.state)&&C.remoteEnabled,L=C.programOptions.filter(A=>x[A.key]),M=[...x1.map(A=>L.find(n=>n.key===A)).filter(Boolean),...L.filter(A=>!x1.includes(A.key))],e=C.state===o.OFF?null:C.program,a=M.map(A=>{let n=A.key===e,m=x[A.key],c=`${H(`program_hint.${A.key}`)} \xB7 ${K(m.water)} l \xB7 ${K(m.energy,3)} kWh \xB7 ${F(m.duration,H)}`;return`<button class="chip" type="button" data-action="program" data-value="${t(A.label)}"
        aria-pressed="${n}" title="${t(c)}" ${V?"":"disabled"}>
        ${d(m.icon)}<span>${t(H(`program.${A.key}`))}</span></button>`}),i=e?t(H(`program_hint.${e}`)):"";return`
      <div class="section">
        <div class="section-title">${t(H("ui.program"))}</div>
        <div class="chips">${a.join("")}</div>
        ${i?`<div class="note">${i}</div>`:""}
      </div>`}_optionsHtml(C,H){if(!C.options.length||C.state===o.OFF)return"";let V=H5.includes(C.state)&&C.remoteEnabled,L=C.options.filter(e=>e.supported||e.on);if(!L.length)return`
        <div class="section">
      ...
      </div>`,A=this._delayNotice&&Date.now()-this._delayNotice.at<2e4?`<div class="note">${d("info")}${t(this._delayNotice.text)}</div>`:"";return`
      <div class="section">
        <div class="section-title">${t(H("ui.delay"))}${e}</div>
        <div class="chips">${M.join("")}</div>
        ${i}
        ${A}
      </div>`}_scoresStale(C){let H=[C.scores.eco,C.scores.energy,C.scores.water].join("/"),V=this._scoreTrack;return V?V.program!==C.program?this._scoreTrack={program:C.program,values:H,stale:!0,since:Date.now()}:V.stale&&(H!==V.values||Date.now()-V.since>3e4)&&(V.stale=!1):this._scoreTrack={program:C.program,values:H,stale:!1,since:0},this._scoreTrack.stale}_metersHtml(C,H){if(C.state===o.OFF)return"";let V=[],{eco:L,energy:M,water:e}=C.scores,a=x[C.program]?.scores!==!1,i=this._scoresStale(C);if(this._config.show_scores&&a&&(L!==null||M!==null||e!==null)){let n=(m,c,Z,v)=>{let h=Math.max(0,Math.min(7,v??0)),b=Array.from({length:7},(z,N)=>`<i class="${N<h?"on":""}"></i>`).join("");return`
          <div class="meter ${c}">
        ...
        </div>`)}if(!V.length)return"";let A=i&&a?`<span class="hint">${t(H("ui.scores_updating"))}</span>`:"";return`
      <div class="section">
        <div class="section-title">${t(H("ui.scores"))}${A}</div>
        ${V.join("")}
          ...
          ${d(a.icon)}<span>${t(H(`command.${e}`))}</span></button>`});if(!L.length)return"";let M=C.remoteEnabled?"":`<div class="note">${d("remote")}${t(H("ui.remote_disabled_hint"))}</div>`;return`<div class="section"><div class="controls">${L.join("")}</div>${M}</div>`}_applianceSettingsHtml(C,H){let{brightness:V,floorLight:L,endSound:M,keyTone:e}=C.settings;if(!V&&!L&&!M&&!e)return"";let a=C.online&&C.remoteEnabled,i=(m,c,Z,v)=>v?`<button class="chip" type="button" data-action="setting" data-value="${m}"
            aria-pressed="${v.on}" title="${t(H(`ui.${Z}_hint`,""))}"
            ${a?"":"disabled"}>
            ${d(c)}<span>${t(H(`ui.${Z}`))}</span>
          </button>`:"",A=[i("floor","floor_light","floor_light",L),i("sound","end_sound","end_sound",M),i("key","key_tone","key_tone",e)].filter(Boolean),n=V?`<div class="steppers narrow">
...
`,s5=i1+U5;var k1={language:"auto",compact:!1,animate:!0,show_methods:!0,show_presets:!0,show_settings:!0,show_probe:!0,show_details:!0,show_controls:!0};function Z5(r,C){if(r==null)return null;let H=Math.max(0,Math.round(r));if(H>=3600){let M=Math.floor(H/3600),e=Math.round(H%3600/60);return{value:`${M}:${String(e).padStart(2,"0")}`,unit:C("ui.hour_short")}}let V=Math.floor(H/60),L=H%60;return{value:`${V}:${String(L).padStart(2,"0")}`,unit:C("ui.minute_short")}}var g1=class extends HTMLElement{static getConfigElement(){return document.createElement(Y)}static getStubConfig(C){let H=n1(C);return{type:`custom:${y}`,device:H[0]?.id}}constructor(){super(),this.attachShadow({mode:"open"}),this._sections={},this._signatures={},this._built=!1}setConfig(C){this._config={...k1,...C||{}},this._signatures={},this._hass&&this._render()}set hass(C){this._hass=C,this._render()}get hass(){return this._hass}connectedCallback(){this._timer=window.setInterval(()=>this._render(),3e4)}disconnectedCallback(){this._timer&&window.clearInterval(this._timer),this._timer=void 0}getCardSize(){return this._config?.compact?5:8}getGridOptions(){return{columns:12,min_columns:6,rows:this._config?.compact?6:"auto"}}_build(){let C=document.createElement("style");C.textContent=s5;let H=document.createElement("ha-card"),V=document.createElement("div");V.className="wrap",V.addEventListener("click",L=>this._onClick(L));for(let L of["header","hero","alerts","gauges","methods","presets","settings","controls","details"]){let M=document.createElement("div");M.dataset.section=L,this._sections[L]=M,V.appendChild(M)}H.appendChild(V),this.shadowRoot.replaceChildren(C,H),this._wrap=V,this._built=!0}_render(){if(!this._hass||!this._config)return;this._built||this._build();let C=d1(p1(this._config,this._hass)),H=l5(this._hass,this._config);if(this._model=H,!H.ok){this._renderEmpty(H,C);return}this._emptyShown=!1,this._applyHostClasses(H),this._section("header",this._headerHtml(H,C)),this._heroSection(H,C),this._section("alerts",this._alertsHtml(H,C)),this._section("gauges",this._gaugesHtml(H,C)),this._section("methods",this._config.show_methods?this._methodsHtml(H,C):""),this._section("presets",this._config.show_presets?this._presetsHtml(H,C):""),this._section("settings",this._config.show_settings?this._settingsHtml(H,C):""),this._section("controls",this._config.show_controls?this._controlsHtml(H,C):""),this._section("details",this._config.show_details?this._detailsHtml(H,C):"")}_renderEmpty(C,H){if(this._emptyShown!==C.reason){this._emptyShown=C.reason,this._signatures={};for(let V of Object.values(this._sections))V.innerHTML="";this._sections.header.innerHTML=`
      ...
      <div class="header">
        ...
        <div class="hero">
          ...
          <div class="status"></div>
        </div>`,this._statusNode=this._sections.hero.querySelector(".status"));let V=this._statusHtml(C,H);this._signatures.status!==V&&(this._signatures.status=V,this._statusNode.innerHTML=V);let L=Z5(C.remaining,H),M=$.includes(C.status)&&L&&C.remaining>0;x5(this._sections.hero,C.status===p.STANDBY||C.status===p.POWERSAVE?"":M?L.value:"--:--",C.currentTemp!==null&&C.status!==p.STANDBY?`${Math.round(C.currentTemp)}\xB0`:"")}_statusHtml(C,H){let V=[],L=C.status===p.PRECOOK&&C.targetTemp?`<span class="phase-text">${Math.round(C.targetTemp)} ${t(C.tempUnit)}</span>`:C.resting?`<span class="phase-text">${t(H("alert.resting"))}</span>`:"";V.push(`
      <div class="status-line">
        ...
        </div>`);let M=Z5(C.remaining,H);if($.includes(C.status)&&M&&C.remaining>0){let e=C.finishAt?`<span class="at">${d("clock")}${t(H("ui.ready_at"))} ${S(C.finishAt,this._hass)}</span>`:"";V.push(`
        <div class="countdown">
        ...
        aria-pressed="${e.key===C.method}" ${A?`title="${t(A)}"`:""}
        ${V?"":"disabled"}>
      ...
      <div class="section">
        ...
        <div class="chips">${L.join("")}</div>
      </div>`}_settingsHtml(C,H){let V=o1.includes(C.status)||C.status===p.PAUSE,L=[],M=(i,A,n,m,c)=>{if(!n||!C.entities[c])return"";let Z=n.value??n.min,v=V&&Z>n.min,h=V&&Z<n.max;return`
        <div class="stepper">
          ...
          ${d("fan")}<span>${t(H("ui.airspeed_high"))}</span></button>`)}if(a.length&&L.push(`<div class="chips">${a.join("")}</div>`),this._config.show_probe&&C.probe.plugged){let i=i5.map(A=>`<div class="row"><span>${t(H(`ui.food.${A.food}`))} \u2013 ${t(H(`ui.doneness.${A.doneness}`))}</span><b>${A.range}</b></div>`).join("");L.push(`
