...
`,g2=h2+q2;function s(r,C){if(!r)return"";let H=C?.locale?.language||C?.language||"hu",V=C?.locale?.time_format,L={hour:"2-digit",minute:"2-digit"};V==="12"&&(L.hour12=!0),V==="24"&&(L.hour12=!1);try{return new Intl.DateTimeFormat(H,L).format(r)}catch{return r.toTimeString().slice(0,5)}}function g(r,C){if(r==null)return null;let H=Math.max(0,Math.round(r));if(H<60)return{value:String(H),unit:C("ui.minute_short")};let V=Math.floor(H/60),L=H%60;return{value:`${V}:${String(L).padStart(2,"0")}`,unit:C("ui.hour_short")}}function w(r,C){let H=g(r,C);return H?`${H.value} ${H.unit}`:""}function U(r){if(r==null)return null;let C=Math.max(0,Math.round(r)),H=Math.floor(C/60),V=C%60;return H?`${H}:${String(V).padStart(2,"0")}`:`0:${String(V).padStart(2,"0")}`}function b(r,C=1){return r==null||!Number.isFinite(r)?"\u2013":r.toFixed(C).replace(/\.0$/,"")}function a(r){return String(r??"").replace(/[&<>"']/g,C=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[C])}var $={language:"auto",compact:!1,animate:!0,show_programs:!0,show_options:!0,show_delay:!0,show_scores:!0,show_consumption:!0,show_details:!0,show_controls:!0,show_appliance_settings:!0},X2=[t.OFF,t.IDLE,t.READY_TO_START],f2=[t.IDLE,t.READY_TO_START],Q=class extends HTMLElement{static getConfigElement(){return document.createElement(y)}static getStubConfig(C){let H=R(C);return{type:`custom:${S}`,device:H[0]?.id}}constructor(){super(),this.attachShadow({mode:"open"}),this._sections={},this._signatures={},this._built=!1}setConfig(C){this._config={...$,...C||{}},this._signatures={},this._hass&&this._render()}set hass(C){this._hass=C,this._render()}get hass(){return this._hass}connectedCallback(){this._timer=window.setInterval(()=>this._render(),3e4)}disconnectedCallback(){this._timer&&window.clearInterval(this._timer),this._timer=void 0}getCardSize(){return this._config?.compact?5:9}getGridOptions(){return{columns:12,min_columns:6,rows:this._config?.compact?6:"auto"}}_build(){let C=document.createElement("style");C.textContent=g2;let H=document.createElement("ha-card"),V=document.createElement("div");V.className="wrap",V.addEventListener("click",L=>this._onClick(L));for(let L of["header","hero","alerts","programs","options","delay","meters","controls","appliance","details"]){let M=document.createElement("div");M.dataset.section=L,this._sections[L]=M,V.appendChild(M)}H.appendChild(V),this.shadowRoot.replaceChildren(C,H),this._card=H,this._wrap=V,this._built=!0}_render(){if(!this._hass||!this._config)return;this._built||this._build();let C=P(this._config,this._hass),H=T(C);this._t=H;let V=n1(this._hass,this._config);if(this._model=V,!V.ok){this._renderEmpty(V,H);return}this._emptyShown=!1,this._applyHostClasses(V),this._section("header",this._headerHtml(V,H)),this._heroSection(V,H),this._section("alerts",this._alertsHtml(V,H)),this._section("programs",this._config.show_programs?this._programsHtml(V,H):""),this._section("options",this._config.show_options?this._optionsHtml(V,H):""),this._section("delay",this._config.show_delay?this._delayHtml(V,H):""),this._section("meters",this._metersHtml(V,H)),this._section("controls",this._config.show_controls?this._controlsHtml(V,H):""),this._section("appliance",this._config.show_appliance_settings?this._applianceSettingsHtml(V,H):""),this._section("details",this._config.show_details?this._detailsHtml(V,H):"")}_renderEmpty(C,H){if(this._emptyShown!==C.reason){this._emptyShown=C.reason,this._signatures={};for(let V of Object.values(this._sections))V.innerHTML="";this._sections.header.innerHTML=`
      ...
      <div class="header">
        ...
        <div class="hero">
          <div class="machine-wrap" data-action="more-info">${l1()}</div>
          <div class="status"></div>
        </div>`,this._statusNode=this._sections.hero.querySelector(".status"));let V=this._statusHtml(C,H);this._signatures.status!==V&&(this._signatures.status=V,this._statusNode.innerHTML=V),v1(this._sections.hero,this._displayText(C,H))}_displayText(C,H){return C.state===t.OFF?"":C.state===t.DELAYED_START&&C.delay?U(C.delay):C.remaining!==null&&[t.RUNNING,t.PAUSED,t.DELAYED_START].includes(C.state)?U(C.remaining):C.state===t.END_OF_CYCLE?"0:00":C.program?H(`program.${C.program}`).slice(0,8):"--:--"}_statusHtml(C,H){let V=[],L=C.state===t.RUNNING&&C.phase!=="UNAVAILABLE"?`<span class="phase-text">${a(H(`phase_long.${C.phase}`))}</span>`:"";V.push(`
      <div class="status-line">
        ...
        <span class="at">${d("timer")}${a(H("ui.duration"))}</span></div>`}return""}_timelineHtml(C,H){let V=m[C.program]?.steps||_,L=C.step?V.indexOf(C.step):-1;return`<div class="timeline">${_.map(e=>{let A=V.includes(e),i=V.indexOf(e),o="step";return A?L>=0&&i<L?o+=" done":L>=0&&i===L?o+=" active":C.state===t.END_OF_CYCLE&&(o+=" done"):o+=" skip",`<div class="${o}"><span class="dot"></span><span class="label">${a(H(`phase.${e}`))}</span></div>`}).join("")}</div>`}_alertsHtml(C,H){let V=[];C.doorOpen&&C.state!==t.OFF&&V.push({severity:"warning",text:H("ui.door_open"),iconName:"door"});for(let L of C.alerts){let{code:M,count:e}=L,i=(L.severity==="ERROR"||L.severity==="CRITICAL"?"error":"")||I[M]||"warning",o=M?H(`alert.${M}`,`${H("alert_generic")}: ${M.replace(/^DISH_ALARM_/,"")}`):`${H("alert_generic")}: ${e}`;V.push({severity:i,text:o,iconName:i==="error"?"error":"alert"})}return C.online||V.push({severity:"error",text:H("ui.offline"),iconName:"wifi_off"}),V.length?`<div class="alerts">${V.map(L=>`<div class="alert ${L.severity}">${d(L.iconName)}<span>${a(L.text)}</span></div>`).join("")}</div>`:""}_programsHtml(C,H){if(!C.entities.program||!C.programOptions.length)return"";let V=X2.includes(C.state)&&C.remoteEnabled,L=C.programOptions.filter(i=>m[i.key]),e=[...N.map(i=>L.find(o=>o.key===i)).filter(Boolean),...L.filter(i=>!N.includes(i.key))].map(i=>{let o=i.key===C.program,l=m[i.key],x=`${H(`program_hint.${i.key}`)} \xB7 ${b(l.water)} l \xB7 ${b(l.energy,3)} kWh \xB7 ${w(l.duration,H)}`;return`<button class="chip" type="button" data-action="program" data-value="${a(i.label)}"
        aria-pressed="${o}" title="${a(x)}" ${V?"":"disabled"}>
        ${d(l.icon)}<span>${a(H(`program.${i.key}`))}</span></button>`}),A=C.program?a(H(`program_hint.${C.program}`)):"";return`
      <div class="section">
        <div class="section-title">${a(H("ui.program"))}</div>
        <div class="chips">${e.join("")}</div>
        ${A?`<div class="note">${A}</div>`:""}
      </div>`}_optionsHtml(C,H){if(!C.options.length)return"";let V=f2.includes(C.state)&&C.remoteEnabled,L=C.options.filter(e=>e.supported||e.on);if(!L.length)return`
        <div class="section">
      ...
      </div>`}_delayHtml(C,H){if(!C.entities.start_time)return"";let V=C.state===t.DELAYED_START||C.delay>0,L=(f2.includes(C.state)||C.state===t.DELAYED_START)&&C.remoteEnabled;if(!L&&!V)return"";let M=V1.map(A=>{let i=C.delay===A;return`<button class="chip" type="button" data-action="delay" data-value="${A}"
      ...
      <div class="section">
        ...
        <div class="chips">${M.join("")}</div>
      </div>`}_metersHtml(C,H){let V=[],{eco:L,energy:M,water:e}=C.scores;if(this._config.show_scores&&(L!==null||M!==null||e!==null)){let A=(i,o,l,x)=>{let f=Math.max(0,Math.min(7,x??0)),Z=Array.from({length:7},(v,B)=>`<i class="${B<f?"on":""}"></i>`).join("");return`
          <div class="meter ${o}">
        ...
        </div>`)}if(this._config.show_consumption&&C.program&&m[C.program]){let A=m[C.program];V.push(`
        <div class="facts">
          <span class="fact">${d("water")}<b>${b(A.water)}</b> l</span>
          <span class="fact">${d("energy")}<b>${b(A.energy,3)}</b> kWh</span>
          <span class="fact">${d("timer")}<b>${a(w(A.duration,H))}</b></span>
        </div>`)}return V.length?`
      <div class="section">
        <div class="section-title">${a(H("ui.scores"))}</div>
        ${V.join("")}
      </div>`:""}_controlsHtml(C,H){let L=(C1[C.state]||[]).filter(e=>C.entities[`cmd_${e}`]).map(e=>{let A=H1[e],i=!C.remoteEnabled&&e!=="off"?"disabled":"";return`<button class="btn ${A.style}" type="button" data-action="command" data-value="${e}" ${i}>
          ${d(A.icon)}<span>${a(H(`command.${e}`))}</span></button>`});if(!L.length)return"";let M=C.remoteEnabled?"":`<div class="note">${d("remote")}${a(H("ui.remote_disabled_hint"))}</div>`;return`<div class="section"><div class="controls">${L.join("")}</div>${M}</div>`}_applianceSettingsHtml(C,H){let{brightness:V,floorLight:L,endSound:M,keyTone:e}=C.settings;if(!V&&!L&&!M&&!e)return"";let A=C.online&&C.remoteEnabled,i=(x,f,Z,v)=>v?`<button class="chip" type="button" data-action="setting" data-value="${x}"
            aria-pressed="${v.on}" title="${a(H(`ui.${Z}_hint`,""))}"
            ${A?"":"disabled"}>
            ${d(f)}<span>${a(H(`ui.${Z}`))}</span>
          </button>`:"",o=[i("floor","floor_light","floor_light",L),i("sound","end_sound","end_sound",M),i("key","key_tone","key_tone",e)].filter(Boolean),l=V?`<div class="steppers narrow">
...
`;function y2(r){return class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}setConfig(H){this._config={...H},this._render()}set hass(H){this._hass=H,this._render()}_render(){if(!this._config||!this._hass)return;let H=r.translator(r.getLanguage(this._config,this._hass)),V=r.listDevices(this._hass),L=this._config.device||"",M=[`<option value="" ${L?"":"selected"}>${a(H("editor.device_auto"))}</option>`,...V.map(i=>`<option value="${a(i.id)}" ${i.id===L?"selected":""}>${a(i.name)}</option>`)].join(""),e=[`<option value="auto">${a(H("editor.language_auto"))}</option>`,...r.languages.map(i=>`<option value="${i}" ${this._config.language===i?"selected":""}>${i.toUpperCase()}</option>`)].join(""),A=r.toggles.map(i=>{let o=this._config[i]??r.defaults[i]??!1;return`<label class="toggle"><input type="checkbox" data-key="${i}" ${o?"checked":""}>
        ...
        </div>`,this.shadowRoot.querySelectorAll("[data-key]").forEach(i=>i.addEventListener("change",o=>this._onChange(o)))}_onChange(H){let V=H.target,L=V.dataset.key,M={...this._config,type:this._config.type||`custom:${r.cardName}`};V.type==="checkbox"?M[L]=V.checked:V.value===""?delete M[L]:M[L]=V.value,this._config=M,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:M},bubbles:!0,composed:!0}))}}}var C5=y2({cardName:S,toggles:["show_programs","show_options","show_delay","show_scores","show_consumption","show_appliance_settings","show_details","show_controls","compact","animate"],defaults:$,listDevices:R,getLanguage:P,translator:T,languages:W});function w2(){customElements.get(y)||customElements.define(y,C5)}w2();k2();
