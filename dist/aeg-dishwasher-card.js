...
`,Z2=l2+z2;function f(M,C){if(!M)return"";let H=C?.locale?.language||C?.language||"hu",V=C?.locale?.time_format,L={hour:"2-digit",minute:"2-digit"};V==="12"&&(L.hour12=!0),V==="24"&&(L.hour12=!1);try{return new Intl.DateTimeFormat(H,L).format(M)}catch{return M.toTimeString().slice(0,5)}}function c(M,C){if(M==null)return null;let H=Math.max(0,Math.round(M));if(H<60)return{value:String(H),unit:C("ui.minute_short")};let V=Math.floor(H/60),L=H%60;return{value:`${V}:${String(L).padStart(2,"0")}`,unit:C("ui.hour_short")}}function k(M,C){let H=c(M,C);return H?`${H.value} ${H.unit}`:""}function S2(M){if(M==null)return null;let C=Math.max(0,Math.round(M)),H=Math.floor(C/60),V=C%60;return H?`${H}:${String(V).padStart(2,"0")}`:`0:${String(V).padStart(2,"0")}`}function y(M,C=1){return M==null||!Number.isFinite(M)?"\u2013":M.toFixed(C).replace(/\.0$/,"")}function A(M){return String(M??"").replace(/[&<>"']/g,C=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[C])}var Q={language:"auto",compact:!1,animate:!0,show_programs:!0,show_options:!0,show_delay:!0,show_scores:!0,show_consumption:!0,show_details:!0,show_controls:!0},G2=[e.OFF,e.IDLE,e.READY_TO_START],u2=[e.IDLE,e.READY_TO_START],U=class extends HTMLElement{static getConfigElement(){return document.createElement(g)}static getStubConfig(C){let H=T(C);return{type:`custom:${l}`,device:H[0]?.id}}constructor(){super(),this.attachShadow({mode:"open"}),this._sections={},this._signatures={},this._built=!1}setConfig(C){this._config={...Q,...C||{}},this._signatures={},this._hass&&this._render()}set hass(C){this._hass=C,this._render()}get hass(){return this._hass}connectedCallback(){this._timer=window.setInterval(()=>this._render(),3e4)}disconnectedCallback(){this._timer&&window.clearInterval(this._timer),this._timer=void 0}getCardSize(){return this._config?.compact?5:9}getGridOptions(){return{columns:12,min_columns:6,rows:this._config?.compact?6:"auto"}}_build(){let C=document.createElement("style");C.textContent=Z2;let H=document.createElement("ha-card"),V=document.createElement("div");V.className="wrap",V.addEventListener("click",L=>this._onClick(L));for(let L of["header","hero","alerts","programs","options","delay","meters","controls","details"]){let r=document.createElement("div");r.dataset.section=L,this._sections[L]=r,V.appendChild(r)}H.appendChild(V),this.shadowRoot.replaceChildren(C,H),this._card=H,this._wrap=V,this._built=!0}_render(){if(!this._hass||!this._config)return;this._built||this._build();let C=B(this._config,this._hass),H=b(C);this._t=H;let V=d1(this._hass,this._config);if(this._model=V,!V.ok){this._renderEmpty(V,H);return}this._emptyShown=!1,this._applyHostClasses(V),this._section("header",this._headerHtml(V,H)),this._heroSection(V,H),this._section("alerts",this._alertsHtml(V,H)),this._section("programs",this._config.show_programs?this._programsHtml(V,H):""),this._section("options",this._config.show_options?this._optionsHtml(V,H):""),this._section("delay",this._config.show_delay?this._delayHtml(V,H):""),this._section("meters",this._metersHtml(V,H)),this._section("controls",this._config.show_controls?this._controlsHtml(V,H):""),this._section("details",this._config.show_details?this._detailsHtml(V,H):"")}_renderEmpty(C,H){if(this._emptyShown!==C.reason){this._emptyShown=C.reason,this._signatures={};for(let V of Object.values(this._sections))V.innerHTML="";this._sections.header.innerHTML=`
      <div class="empty">
        ${d("dishwasher")}
        <div>${A(H(`ui.${C.reason}`))}</div>
      </div>`}}_section(C,H){this._signatures[C]!==H&&(this._signatures[C]=H,this._sections[C].innerHTML=H)}_applyHostClasses(C){let H=["wrap"];this._config.compact&&H.push("compact"),this._config.animate||H.push("no-animation"),H.push(`accent-${C.accent}`),C.state===e.RUNNING&&H.push("running","busy"),C.state===e.PAUSED&&H.push("paused"),C.state===e.END_OF_CYCLE&&H.push("done"),C.state===e.OFF&&H.push("off"),(C.state===e.ALARM||C.alerts.some(L=>I[L]==="error"))&&H.push("alarm"),C.step==="DRYING"&&H.push("phase-drying"),C.doorOpen&&H.push("door-open");let V=H.join(" ");this._wrap.className!==V&&(this._wrap.className=V)}_headerHtml(C,H){let V=A(C.name||H("card_name")),L=[e.IDLE,e.READY_TO_START].includes(C.state)&&C.minutesToFinish?C.minutesToFinish:p[C.program]?.duration,r=C.program&&L?`${H(`program.${C.program}`)} \xB7 ${k(L,H)}`:C.programRaw||"",t=[];return C.online?C.linkQuality&&t.push(`<span title="${H("ui.wifi")}: ${C.linkQuality}">${d("wifi")}</span>`):t.push(`<span class="bad" title="${H("ui.offline")}">${d("wifi_off")}</span>`),C.doorOpen&&t.push(`<span class="bad" title="${H("ui.door_open")}">${d("door")}</span>`),C.ecoMode&&t.push(`<span title="${H("ui.eco_mode")}">${d("leaf")}</span>`),`
      <div class="header">
        <div>
        ...
        <div class="hero">
          <div class="machine-wrap" data-action="more-info">${m1()}</div>
          <div class="status"></div>
        </div>`,this._statusNode=this._sections.hero.querySelector(".status"));let V=this._statusHtml(C,H);this._signatures.status!==V&&(this._signatures.status=V,this._statusNode.innerHTML=V),n1(this._sections.hero,this._displayText(C,H))}_displayText(C,H){return C.state===e.OFF?"":C.remaining!==null&&[e.RUNNING,e.PAUSED,e.DELAYED_START].includes(C.state)?S2(C.remaining):C.state===e.END_OF_CYCLE?"0:00":C.program?H(`program.${C.program}`).slice(0,8):"--:--"}_statusHtml(C,H){let V=[],L=C.state===e.RUNNING&&C.phase!=="UNAVAILABLE"?`<span class="phase-text">${A(H(`phase_long.${C.phase}`))}</span>`:"";V.push(`
      <div class="status-line">
        ...
        </div>`);let r=this._countdownHtml(C,H);return r&&V.push(r),[e.RUNNING,e.PAUSED,e.END_OF_CYCLE].includes(C.state)&&(V.push(`<div class="bar"><i style="width:${Math.round(C.progress*100)}%"></i></div>`),V.push(this._timelineHtml(C,H))),V.join("")}_countdownHtml(C,H){if(C.state===e.END_OF_CYCLE)return`<div class="countdown"><span class="at">${d("check")}${A(H("ui.finished_hint"))}</span></div>`;if(C.state===e.DELAYED_START){let V=c(C.delay||C.remaining,H);if(!V)return"";let L=[C.startAt?`<span class="at">${d("timer")}${A(H("ui.starts_at"))} ${f(C.startAt,this._hass)}</span>`:"",C.finishAt?`<span class="at">${d("clock")}${A(H("ui.ready_at"))} ${f(C.finishAt,this._hass)}</span>`:""].join("");return`<div class="countdown"><span class="value">${V.value}</span><span class="unit">${V.unit}</span>${L}</div>`}if([e.RUNNING,e.PAUSED].includes(C.state)){let V=c(C.remaining,H);if(!V)return"";let L=C.finishAt?`<span class="at">${d("clock")}${A(H("ui.ready_at"))} ${f(C.finishAt,this._hass)}</span>`:"";return`<div class="countdown"><span class="value">${V.value}</span><span class="unit">${V.unit}</span>${L}</div>`}if([e.IDLE,e.READY_TO_START].includes(C.state)){let V=c(C.minutesToFinish??p[C.program]?.duration,H);if(!V)return"";let L=C.finishAt?`<span class="at">${d("clock")}${A(H("ui.ready_at"))} ${f(C.finishAt,this._hass)}</span>`:"";return`<div class="countdown"><span class="value">${V.value}</span><span class="unit">${V.unit}</span>${L}</div>`}if(C.state===e.OFF&&C.program&&p[C.program]){let V=c(p[C.program].duration,H);return`<div class="countdown"><span class="value">${V.value}</span><span class="unit">${V.unit}</span>
        <span class="at">${d("timer")}${A(H("ui.duration"))}</span></div>`}return""}_timelineHtml(C,H){let V=p[C.program]?.steps||_,L=C.step?V.indexOf(C.step):-1;return`<div class="timeline">${_.map(t=>{let a=V.includes(t),i=V.indexOf(t),o="step";return a?L>=0&&i<L?o+=" done":L>=0&&i===L?o+=" active":C.state===e.END_OF_CYCLE&&(o+=" done"):o+=" skip",`<div class="${o}"><span class="dot"></span><span class="label">${A(H(`phase.${t}`))}</span></div>`}).join("")}</div>`}_alertsHtml(C,H){let V=[];C.doorOpen&&C.state!==e.OFF&&V.push({severity:"warning",text:H("ui.door_open"),iconName:"door"});for(let L of C.alerts){let r=I[L]||"warning",t=H(`alert.${L}`,`${H("alert_generic")}: ${L}`);V.push({severity:r,text:t,iconName:r==="error"?"error":"alert"})}return C.online||V.push({severity:"error",text:H("ui.offline"),iconName:"wifi_off"}),V.length?`<div class="alerts">${V.map(L=>`<div class="alert ${L.severity}">${d(L.iconName)}<span>${A(L.text)}</span></div>`).join("")}</div>`:""}_programsHtml(C,H){if(!C.entities.program||!C.programOptions.length)return"";let V=G2.includes(C.state)&&C.remoteEnabled,L=C.programOptions.filter(i=>p[i.key]),t=[...N.map(i=>L.find(o=>o.key===i)).filter(Boolean),...L.filter(i=>!N.includes(i.key))].map(i=>{let o=i.key===C.program,v=p[i.key],m=`${H(`program_hint.${i.key}`)} \xB7 ${y(v.water)} l \xB7 ${y(v.energy,3)} kWh \xB7 ${k(v.duration,H)}`;return`<button class="chip" type="button" data-action="program" data-value="${A(i.label)}"
        aria-pressed="${o}" title="${A(m)}" ${V?"":"disabled"}>
        ${d(v.icon)}<span>${A(H(`program.${i.key}`))}</span></button>`}),a=C.program?A(H(`program_hint.${C.program}`)):"";return`
      <div class="section">
        <div class="section-title">${A(H("ui.program"))}</div>
        <div class="chips">${t.join("")}</div>
        ${a?`<div class="note">${a}</div>`:""}
      </div>`}_optionsHtml(C,H){if(!C.options.length)return"";let V=u2.includes(C.state)&&C.remoteEnabled,L=C.options.filter(t=>t.supported||t.on);if(!L.length)return`
        <div class="section">
      ...
      </div>`}_delayHtml(C,H){if(!C.entities.start_time)return"";let V=C.state===e.DELAYED_START||C.delay>0,L=(u2.includes(C.state)||C.state===e.DELAYED_START)&&C.remoteEnabled;if(!L&&!V)return"";let r=H1.map(a=>{let i=C.delay===a;return`<button class="chip" type="button" data-action="delay" data-value="${a}"
        aria-pressed="${i}" ${L?"":"disabled"}>
        ${d("timer")}<span>${a/60} ${A(H("ui.hour_short"))}</span></button>`});V&&r.push(`<button class="chip" type="button" data-action="delay" data-value="-1" ${L?"":"disabled"}>
        ${d("stop")}<span>${A(H("ui.delay_cancel"))}</span></button>`);let t=V?`<span class="hint">${A(k(C.delay,H))}</span>`:`<span class="hint">${A(H("ui.delay_off"))}</span>`;return`
      <div class="section">
        <div class="section-title">${A(H("ui.delay"))}${t}</div>
        <div class="chips">${r.join("")}</div>
      </div>`}_metersHtml(C,H){let V=[],{eco:L,energy:r,water:t}=C.scores;if(this._config.show_scores&&(L!==null||r!==null||t!==null)){let a=(i,o,v,m)=>{let R=Math.max(0,Math.min(7,m??0)),O=Array.from({length:7},(u,w)=>`<i class="${w<R?"on":""}"></i>`).join("");return`
          <div class="meter ${o}">
        ...
        <div class="meters">
          ...
          ${a("water_score","water","water",t)}
        </div>`)}if(this._config.show_consumption&&C.program&&p[C.program]){let a=p[C.program];V.push(`
        <div class="facts">
          <span class="fact">${d("water")}<b>${y(a.water)}</b> l</span>
          <span class="fact">${d("energy")}<b>${y(a.energy,3)}</b> kWh</span>
          <span class="fact">${d("timer")}<b>${A(k(a.duration,H))}</b></span>
        </div>`)}return V.length?`
      <div class="section">
        <div class="section-title">${A(H("ui.scores"))}</div>
        ${V.join("")}
      </div>`:""}_controlsHtml(C,H){let L=(J[C.state]||[]).filter(t=>C.entities[`cmd_${t}`]).map(t=>{let a=C1[t],i=!C.remoteEnabled&&t!=="off"?"disabled":"";return`<button class="btn ${a.style}" type="button" data-action="command" data-value="${t}" ${i}>
          ${d(a.icon)}<span>${A(H(`command.${t}`))}</span></button>`});if(!L.length)return"";let r=C.remoteEnabled?"":`<div class="note">${d("remote")}${A(H("ui.remote_disabled_hint"))}</div>`;return`<div class="section"><div class="controls">${L.join("")}</div>${r}</div>`}_detailsHtml(C,H){let V=[],L=(r,t,a)=>{t==null||t===""||V.push(`
        <div class="detail">
...
`;function c2(M){return class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}setConfig(H){this._config={...H},this._render()}set hass(H){this._hass=H,this._render()}_render(){if(!this._config||!this._hass)return;let H=M.translator(M.getLanguage(this._config,this._hass)),V=M.listDevices(this._hass),L=this._config.device||"",r=[`<option value="" ${L?"":"selected"}>${A(H("editor.device_auto"))}</option>`,...V.map(i=>`<option value="${A(i.id)}" ${i.id===L?"selected":""}>${A(i.name)}</option>`)].join(""),t=[`<option value="auto">${A(H("editor.language_auto"))}</option>`,...M.languages.map(i=>`<option value="${i}" ${this._config.language===i?"selected":""}>${i.toUpperCase()}</option>`)].join(""),a=M.toggles.map(i=>{let o=this._config[i]??M.defaults[i]??!1;return`<label class="toggle"><input type="checkbox" data-key="${i}" ${o?"checked":""}>
        ...
        </div>`,this.shadowRoot.querySelectorAll("[data-key]").forEach(i=>i.addEventListener("change",o=>this._onChange(o)))}_onChange(H){let V=H.target,L=V.dataset.key,r={...this._config,type:this._config.type||`custom:${M.cardName}`};V.type==="checkbox"?r[L]=V.checked:V.value===""?delete r[L]:r[L]=V.value,this._config=r,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:r},bubbles:!0,composed:!0}))}}}var Q2=c2({cardName:l,toggles:["show_programs","show_options","show_delay","show_scores","show_consumption","show_details","show_controls","compact","animate"],defaults:Q,listDevices:T,getLanguage:B,translator:b,languages:W});function O2(){customElements.get(g)||customElements.define(g,Q2)}O2();s2();
